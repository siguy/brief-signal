...
function main() {
  ...
  const highSignal = entries.filter((e) => e.editorialSignal === "HIGH");
  if (highSignal.length) {
    const uncited = highSignal.filter((e) => cited(e, urls) === false);
    out.push("### HIGH editorial-signal episodes (advisory)");
    out.push("");
    out.push(
      urls && uncited.length
        ? `${uncited.length} of ${highSignal.length} not cited in the draft:\n` +
            uncited.map((e) => line(e, urls, "GCP")).join("\n")
        : urls
