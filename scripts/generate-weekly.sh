...
# Editorial section — the whole Stage 4a decision, rendered by
# scripts/lineup-digest.js from the lineup file (already committed alongside the
# draft); no separate log file to keep in sync. It carries what was selected and
# why, the recommended reads that earned no slot, the cut ledger with Stage 4a's
# own quality rating per reject, and the proposed registry.
#
...
# The same sweep, reduced to its misses, rendered UNCOLLAPSED right under the
# editorial section. The full digest above is a lookup table — it answers "was X
# covered?" — but the reviewer's question at the gate is "did we drop something
# good?", and that answer was reachable only by expanding a <details> and
# scanning several hundred lines. It is also the independent half of the pair:
# the lineup's own cut ledger is Stage 4a rating its own rejects, while this
# reads the KBs' grades and the lineup's URLs with no model involved. They sit
# together on purpose, and where they disagree the disagreement is the finding.
#
# Same non-fatal contract as everything else here: a summary that fails costs
# the section, never the PR.
#
# The failure branch ends on an ASSIGNMENT, never on `log`. `log` pipes through
# `tee -a "$LOG_FILE"`, so it returns non-zero the moment that file stops being
# writable — and a function-final non-zero under `set -e` aborts the run here,
# two lines before `gh pr create`. That is the exact bug scripts/generate-weekly.test.sh
# was written for: "lint found problems, surface them in the PR" became "no PR at all".
#
# It also says the sweep failed rather than rendering nothing. A missing all-clear
# reads as an all-clear, which is the same overclaim renderSummary refuses to make
# when it has nothing to sweep.
SIGNAL_SUMMARY=""
if SUMMARY_OUT=$(node scripts/signal-digest.js "${SIGNAL_ARGS[@]}" --summary 2>&1) && [ -n "$SUMMARY_OUT" ]; then
  SIGNAL_SUMMARY=$'\n\n'"$SUMMARY_OUT"
else
  log "WARN: Signal summary failed — PR body will say the sweep did not run."
  SIGNAL_SUMMARY=$'\n\n### 🔎 Left on the floor — deterministic sweep\n\n_Sweep failed to run, so nothing here says anything was or was not dropped. Check the log, then `npm run signal -- --summary` by hand._'
fi
...
### What to do
1. Read **The editorial decision** below: what was selected and on what grounds, what
   Stage 4a recommends as worth reading anyway, and what it cut.
2. Check the two exclusion views against each other — **Cut** is Stage 4a rating its own
   rejects, **Left on the floor** is the same week swept mechanically off the KBs' grades.
   Anything HIGH in either is a second look.
3. Edit \`${LINEUP_FILE}\` directly if the selection is wrong — add, cut, or re-merge
   stories, or promote a recommended read into Quick Hits.
...
inform that one call.${THEME_SECTION}${SIGNAL_SUMMARY}${SIGNAL_SECTION}
PREOF
)"
else
...
- [ ] Check the editorial decision below — the selection, the recommended reads, and anything rated HIGH that got cut
...
Subscriber email is sent when the audio PR is merged.${THEME_SECTION}${SIGNAL_SUMMARY}${CRITIQUE_SECTION}${SIGNAL_SECTION}
PREOF
)"
fi
