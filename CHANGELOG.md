...
### Fixed
- **GitHub Pages deploy unblocked** (`scripts/fetch-bookmarks.py`) — `npm test` gates the deploy, and it imports `fetch-bookmarks.py`, which imported `twikit` at module scope for its 2.3.3 regex monkey patch. The CI runner has no twikit, so the import raised `ModuleNotFoundError`, `npm test` exited 1, and **the site stopped deploying entirely** — three consecutive failed deploys (#123, #125, #127), last success 2026-08-24, with the live site sitting on Edition #27 while later merges silently never shipped. The two helpers under test (`load_existing_keys`, `should_stop_early`) need nothing but the stdlib, so twikit/dotenv imports and the monkey patch moved into the functions that actually use them: `_patch_twikit()` (idempotent, called at the top of `fetch_all_bookmarks()`) and a local import in `has_media_type()`. Installing twikit in CI was rejected — paying a network install on every deploy to satisfy an import nothing under test calls is the wrong trade, and a real run still fails immediately and loudly on a missing dependency, just not at import time. Patch body verified byte-identical to the original. Pinned by a new subprocess test that hides twikit and dotenv behind an import hook and asserts the module still loads; confirmed to FAIL when a module-scope `from twikit import Client` is reintroduced.

### Added
- **Braid ledger — `npm run braids`** (`scripts/braid-ledger.js`) — compares Stage 4a's *plan* against Stage 4b's *prose*. The lineup's `braids in:` field is the only place the pipeline records its own intent — which bookmarks each Big Picture story should weave in alongside its anchor — and nothing downstream checked whether they arrived. `lint-briefing.js` grades the finished draft; `signal-digest.js` asks what the KBs held that the draft never cited; neither asks *did we do what we said we'd do?* On Edition #28 the answer was 7 of 10. Three outcomes, not two: **landed**, **dropped**, and **substituted** — the subject ran but under a different citation, which is what happened to @SemiAnalysis_'s Jalapeño benchmark (the story reached Quick Hits credited to OpenAI's own announcement rather than to the analysis that made the claim). A yes/no check would have called that a miss and been wrong. Two braids were genuinely dropped: @mardehaym's agentic-org maturity model and @quxiaoyin's 75x task-cost spread. Advisory and never fatal (a story legitimately tightens while drafting); renders into the weekly PR body beside the theme section, ahead of the critique and digest; skipped under `LINEUP_GATE=1`, where there is no draft to compare against. No LLM and no dependencies.
- **Lineup format instability surfaced** — building the ledger showed Stage 4a has emitted **three different `braids in:` shapes** across five editions: `@handle (description)` (#28), `[@handle (1 min read)](url)` (#25, where the parenthetical is a read time and not a description), and prose naming shows with no handle at all (#26: *"Hard Fork (2026-08-21) on Spirit Airlines data"*). The prose form has nothing to match a permalink against, so 8 of that week's 9 braids are unreadable. The ledger counts and lists them as **unreadable** rather than silently under-reporting — a checker claiming 1 planned braid where the lineup planned 9 is worse than one that admits it cannot read the line. **The durable fix is to pin the format in `scripts/briefing-prompt.md`; not yet done.**
