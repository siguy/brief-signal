...
- **"Your angle with founders" is now a rep toolkit, not a quiz** (`scripts/briefing-prompt.md`) — replaces the retired "2 escalating questions + one opportunity line" spec with the four-move shape shipped in Edition #24: **concede what's true → move to an eval on their workload/TCO → compete where the news can't reach → "Where GCP wins:"**. At most ONE question in the whole block, and only if it reframes. Old spec replaced, not appended (zero stale references remain); the concede-when-a-competitor-wins rule extends Principle 5 in place rather than duplicating it; never assume a founder will fine-tune/distill. Verified with a live Gemini run against Edition #24's own lineup so only the prompt differed: 3/3 blocks in the new shape, 208-285 words, zero questions.
- **LLM critique demoted to advisory** (`scripts/generate-weekly.sh`) — the repair pass now fires on deterministic lint hard-failures ONLY. An LLM grading an LLM's output and thereby firing a third LLM to rewrite it is a loop with no ground truth in it. The critique still runs and its full text still lands in the PR body (its coverage check is the most useful thing it produces); it just can't trigger an unattended rewrite. Evidence across all 12 logs in `scripts/logs/`: six critiques instruct the model to use "GEAP" (which `lint-briefing.js:208` hard-fails on), one flags the required "Where the GCP opportunity is" line as a violation, and four demand a "Try This Week" section retired from the prompt.
...
- **A source that ran and found nothing blocked the whole pipeline** (`scripts/extract-podcasts.js`, `scripts/extract-rss-podcasts.py`, `scripts/generate-briefing.js`, `scripts/signal-digest.js`) — the freshness guard proves an extractor ran by its KB file's mtime, so an extractor exiting early **without writing** was indistinguishable from one that crashed. That is what killed the 2026-08-02 run (`generate-2026-08-02.log:415`): the playlist had no new videos, wrote nothing, and the pipeline fell back to a hand-driven Stage 4 with 2 sources instead of 3 — Edition #24 missed its biggest stories as a direct result. Fixed in the producers, not the guard (`check_kb_fresh` is unchanged, and a missing file still blocks): both podcast extractors always leave a dated KB behind, marked `> **Status:** EMPTY — no new items this week.` The two share one file, so the rule is create-if-missing, touch-if-present. Consumers exclude empty sources from the draft and keep them out of the trailing `*Sources:*` line; the digest reports "ran, no new items" instead of "0 entries". The out-of-repo extraction skills carry the same rule — `extract-playlist` previously said, verbatim, *"if it's zero, log a warning and stop — don't generate an empty KB."*
- **A format change silently switched off a lint rule** (`scripts/lint-briefing.js`) — `checkAngleBlocks` matched `**Your angle with founders:**` with the trailing colon **required**. Edition #23 shipped that punctuation and was checked; Edition #24 shipped it without the colon and was **not** — nothing verified its closing line. The heading now matches with the colon optional, and both closing labels are accepted (`Where GCP wins:` current, `Where the GCP opportunity is` through #23). Three regression tests cover both punctuations and both labels.
