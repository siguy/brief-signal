...
### Added
- **Recommended reads — the third disposition** (`scripts/generate-briefing.js` Stage 4a prompt, `scripts/lineup-digest.js`) — a candidate used to have two fates, "ran" or "cut", and high-value items that fit neither (the strategic argument behind an event we ran, a deep technical piece too long for a one-liner, a primary source the edition cites only second-hand) fell into the cut pile with everything else. Stage 4a now nominates 3-5 as **recommended reads** with what the reader gets and why it lost its slot, rendered as its own section in the PR body. Checkpoint-only by deliberate scope: nothing is published, and promoting one is editing the lineup's Quick Hits plus a redraft.
- **Cut ledger with a per-item quality rating** (`scripts/generate-briefing.js` Stage 4a prompt, `scripts/lineup-digest.js`) — "Considered but cut (and why)" has been written by Stage 4a since the lineup pass existed and was rendered **nowhere**: answering "is anything good in the reject pile?" meant opening the lineup file and reading to the bottom. It now renders in the PR body, carries a URL per item, and is sorted by a `quality:` rating that is explicitly about the item's own merit rather than whether it won — so a HIGH item cut for a good reason stays visibly HIGH. HIGH cuts lead the section as the second-look pile; the rest collapses behind a `<details>`. The rating is Stage 4a's claim about its own rejects, not a check — it earns its place because it is the only signal that exists for bookmarks, which carry no grades at all.
- **`npm run signal -- --summary`** (`scripts/signal-digest.js`) — the same deterministic sweep reduced to its misses: HIGH editorial-signal episodes, first-party and Google-specific items that did not reach the edition, and a lab-news count. Renders **uncollapsed** in the PR body directly under the editorial section. The full digest is a lookup table behind a `<details>` — good for "was X covered?", useless for "did we drop something good?", which is the question at the gate. It is also the independent half of the exclusion pair: no model in the loop, so where it and Stage 4a's own cut ledger disagree, the disagreement is the finding. Lane membership is now computed once and shared, so the summary can never report a different set than the table it summarises.
...
### Changed
- **The PR body's editorial section now shows the whole decision** (`scripts/lineup-digest.js`, `scripts/generate-weekly.sh`) — it rendered a story title, `advances:` and `gravity:` and nothing else, so a reviewer could see which arc a story moved but not the argument for running it, and the Quick Hits — half the edition's content — never appeared at all. It now leads with a counts line (Big Picture · Quick Hits · recommended reads · cut, with HIGH cuts called out), then every field Stage 4a wrote per story (event, what changed this week, arc, gravity, seller play, merges, braid), the lead rationale and continuity lines, the Quick Hits inline, the recommended reads and the cut ledger. Simon's complaint about the checkpoint was that it "doesn't give me enough info about what's been selected, what's been excluded but high quality" — all of it was already in the lineup file; none of it was rendered.
- **Stage 4b no longer pays for the review notes** (`scripts/generate-briefing.js`) — `stripRegistryFooter` cut from `**Proposed registry update:**` onward, so the cut ledger and the coverage self-check rode into the prose pass as input on every run. Everything written for the human now lives under a `## Editorial review notes` heading and is stripped at the first review-only marker found. That keeps the enriched ledger (URLs + a quality rating per reject) free at the checkpoint instead of billing it twice — the exact waste the removed HIGH-signal disposition block used to cause. Lineups written before the heading existed fall through to the older markers and strip exactly as they always did, so archived lineups stay redraftable.
...
### Documentation
- **Editorial process — what the checkpoint shows and why** (`docs/editorial-process.md`) — documents the three questions the PR body's editorial section answers in order (what got in and on what grounds / what's worth reading that didn't run / what got cut and was any of it good), and why the two exclusion views are kept apart rather than merged: the cut ledger is Stage 4a rating its own rejects, the left-on-the-floor sweep is arithmetic over grades assigned before any selection happened. Adds step 5a to the order-of-operations table — every loser is written down, nothing leaves the lineup unaccounted for.
