...
-   **Concede the constraint, then make it concrete.** Public resistance is now a real input to how fast anyone adds capacity — Google included. Ask: "Which of your workloads actually have to sit in a specific region, and which could run anywhere?" Most founders have never separated the two, and the answer decides how exposed they are.
-   **Treat siting risk as schedule risk.** A moratorium does not cancel capacity; it moves its date. Walk them through what a two-quarter permitting slip in a region they are counting on would do to a launch date, a burn plan, or a funding milestone. That turns a political story into a planning input they actually own.
...
-   **Where GCP wins:** every hyperscaler is supply-constrained, Google included — so the honest edge is not more capacity, it is more places to put a workload: multiple regions, multiple model tiers, and commitment terms that sell certainty instead of promising abundance.
...
-   **Where GCP wins:** Agent Runtime keeps sessions, memory and secrets outside the model, and Model Garden makes the swap a routing decision rather than a migration — including down to open weights like Gemma the founder runs themselves, the one fallback immune to another vendor's incident.
