...
-   **Abridge's distillation playbook runs end-to-end on GEAP.** Claude Opus → open-source student (Gemma 4) is the GEAP-only differentiator — Bedrock distillation can't pair an Anthropic teacher with a non-Anthropic open-source student. Gemini cannot be a student of an Opus distillation; the student is always an open-source model.
...
The detail worth landing in a founder conversation: **GEAP is the only hyperscaler where Claude Opus can be the teacher for an open-source student model like Gemma**. AWS Bedrock distillation locks you into Anthropic teaching Anthropic. Important nuance — Gemini cannot be the student of an Opus distillation; the student has to be an open-source model the customer owns and runs. For most regulated-vertical founders, that's actually the point: they want to *own* the deployed model, not rent it from any frontier vendor. Pick the highest-accuracy teacher (often Claude for medical or legal language), distill to Gemma running in your VPC for the production loop. GEAP wraps the whole thing in VPC Service Controls, BAA-covered storage, and confidential-compute inference on Blackwell GPUs (the inference layer still Preview), which is the part that makes the HIPAA story actually defensible.
