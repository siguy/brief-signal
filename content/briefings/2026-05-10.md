...
- **GCP plays this week:** AWS-Bedrock-to-GEAP migrations (same Anthropic models, less single-vendor exposure), provisioned throughput contracts on Gemini, the Agentic Data Cloud for siloed-data conversations, and Embedding 2's multimodal advantage (no AWS/Azure equivalent).
...
**Where the GCP opportunity is:** "Build faster" isn't a Code Assist pitch — it's a question about which *decisions* are slow. If the answer is **data access** (siloed data across SaaS systems blocking AI workflows), this is the new **Agentic Data Cloud** conversation: BigQuery + Spanner + AlloyDB + Looker + Dataplex unified for agents, with the Knowledge Catalog federating context across Salesforce, SAP, Workday, and Palantir, and zero-ETL access via Iceberg REST Catalog + Cross-Cloud Interconnect [What's new in the Agentic Data Cloud](https://cloud.google.com/blog/products/data-analytics/whats-new-in-the-agentic-data-cloud). If the answer is "we don't know what the customer wants", offer AI CEs or partners as a research accelerant.
