...
- **Amazon brought conversational ads into ChatGPT**, while Marc Andreessen backed Cognition at a $48B valuation with Devin generating 90% of internal code.
...
- **The question to leave behind:** "If your model vendor launches a direct competitor to your core product next quarter, are they doing it with insights derived from your own API traffic?"
...
- **Where GCP wins:** Model Garden lets founders implement dynamic 80/90 routing across Gemini 3.5 Flash-Lite, Gemma 2, and Claude under a single SDK, while Vertex AI Vector Search and BigQuery Vector Indexing slash prompt bloat at scale.
...
**The behavior change.** Stop debating raw leaderboard scores in founder meetings. Ask: *"What is your insulation strategy when your model provider launches a first-party tool in your category?"* Reframe the decision around platform neutrality. Show them how building on the Gemini Enterprise Agent Platform (FKA Vertex AI) with Model Garden lets them orchestrate Gemini, Claude, and open weights like Gemma side by side—keeping their telemetry sovereign and their architecture vendor-agnostic.

## Our Play

Every thread this edition—the ZDR sovereign migration, the 80/90 token split, and isolated cloud microVMs—points to one GCP position: **Google Cloud is the neutral, sovereign infrastructure partner that powers AI builders without competing against them or trapping their data.** Three concrete motions:

- **Lock down sovereign data with private Agent Platform endpoints.** Enterprise audit committees fear API telemetry leaks and vendor competition. The Agent Platform runs models inside Customer VPCs with CMEK encryption and Private Service Connect, guaranteeing zero telemetry harvesting. Propose a VPC-SC security architecture review to enterprise CISOs migrating off public endpoints.
- **Implement the 80/90 token split in Model Garden.** Unoptimized prompt stuffing is exhausting startup budgets. Model Garden allows dynamic routing between Gemini 3.5 Flash-Lite, Gemma 2, and Claude under one API with 90% implicit context caching. Audit the founder's call traces live to route routine sub-tasks to Flash.
- **Isolate agent fleets on Cloud Run Sandboxes.** Multi-agent background execution requires kernel-level sandbox boundaries. Cloud Run Sandboxes provide gVisor-backed workload isolation and ephemeral microVM execution with Workload Identity Federation for short-lived, least-privilege cloud access. Whiteboard migrating self-hosted agent VMs to managed Cloud Run Sandboxes.
