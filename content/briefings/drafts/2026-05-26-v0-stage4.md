...
featured_topics:
  ...
  - anthropic-engineers-prompt-skills-not-claude
...
-   **From prompts to skills: A new developer workflow.** Anthropic engineers now build composable, self-improving "skills" instead of one-off prompts, signaling a shift to application-layer AI.
...
**Where the GCP opportunity is:** Compute infrastructure and MLOps. Wayve's pain points (data cleanliness, reliability, iteration speed) map directly to Vertex AI's managed services, MLOps tools, and BigQuery. When a founder focuses solely on model benchmarks, pivot to the underlying platform that makes those benchmarks matter in production.
...
**Where the GCP opportunity is:** Long-dated committed-capacity contracts on TPUs and next-gen GPUs. Emphasize our MLOps platform for inference (`Vertex AI Model Serving`) which provides the "sticky software layer" beyond commodity GPUs. Highlight our commitment to internal chip development (TPUs) and deep systems engineering to address runtime and kernel-level challenges at scale.

## Builder's Corner

### From Prompts to Skills: How Anthropic Engineers Build

![anthropic-prompt-skills](./images/anthropic-prompt-skills.jpg)

Anthropic's own engineers have fundamentally shifted how they prompt Claude Code, moving away from one-off prompts to building composable, self-improving "skills." These skills are folders of procedural knowledge with three layers: a description (when to invoke), instructions (the playbook), and tools (scripts, APIs, references). The key insight: most people stop at instructions, but the real leverage is in the tools layer. After every skill run, they ask: "is this fix one-time or forever?" If forever, they update the skill, allowing them to iterate and improve their agentic workflows continuously [Austin Marchese on YouTube (8 min watch)](https://www.youtube.com/watch?v=qOvc9IUKEIc).

**Why founders care:** This is the playbook for building durable, scalable agentic workflows, moving from prompt engineering as an art to a systematic engineering discipline. It directly improves reliability, reusability, and maintainability of AI-powered applications.
...
Pick one founder building agentic workflows and ask them: *"Are you thinking about agent prompts as one-off commands, or are you actively building a library of composable 'skills' with distinct instructions, tools, and continuous self-improvement loops?"* If they're still in the "prompt engineering" mindset, share the Anthropic engineers' playbook.
